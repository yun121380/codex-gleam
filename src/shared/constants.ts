 ...
 * 拦截清单最多留多少条。
 *
 * 一个正常运行的应用这一栏应该是**空的**。50 条已经足够让「不正常」显形了——
 * 再多也只是同一件事重复，而清单本身只是给人看的样本，计数不受它影响。
 */
export const SELF_CHECK_MAX_BLOCKED = 50

/**
 * 「你自己试」的授权窗口，毫秒。
 *
 * 一次 `Image` 请求从设 `src` 到被拦下来是毫秒级的事，10 秒是给慢机器留的余量，
 * 不是给「稍后再来一发也算」留的口子。窗口本身是必需的而不是优化：CSP 有可能在
 * 渲染进程就把那次加载挡掉，请求压根走不到 `onBeforeRequest`，那时授权会一直悬着，
 * 直到某个真正来自应用的请求撞上同一个地址、被错记成用户触发的。
 */
export const SELF_CHECK_PROBE_WINDOW_MS = 10_000

/**
 * 读构建期证据文件的字节上限。
 *
 * 依赖树带上全部传递依赖能有几 MB，8 MB 是留了余量的上界，
 * 同时不至于让一个被写坏的文件把内存吃光。
 */
export const SELF_CHECK_MAX_EVIDENCE_BYTES = 8 * 1024 * 1024

/**
 * 拍平后进报告的包数上限。
 *
 * 这个仓库连开发依赖一共千把个，2000 留了一倍余量。超了就标 `packagesTruncated`，
 * 而 `packageCount` 照旧报截断前的真实条数。
 */
export const SELF_CHECK_MAX_PACKAGES = 2000

/**
 * 用户填的地址长度上限。
 *
 * 纯粹是别让一个几十万字符的输入进到拦截清单里。
 */
export const SELF_CHECK_MAX_URL_LENGTH = 2048
