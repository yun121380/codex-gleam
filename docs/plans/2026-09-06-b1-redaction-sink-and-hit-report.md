...
- [ ] **Step 2: 密钥本体要足够长且形状够真，而且这两件事本身要有断言看着。** 每个假密钥都得真的能被对应规则认出来（否则这条测试在验一件没发生的事），同时**本体不能短到会在别处偶然出现**——比如别拿 `abc123` 当密钥本体，`not.toContain('abc123')` 会因为一个无关的路径片段而挂掉，然后有人为了让它绿而放宽断言。这两条要求都不能只写在注释里：单独一条断言把清单里的每个本体在 fixture 原文里 `toContain` 一遍、并各自过一道最小长度。搜一个 fixture 里压根不存在的串永远搜不到——少了这道检查，将来谁改了 fixture、换掉某个假密钥的名字，下面几条会**静默变成空验**，而空验是绿的。
...
- [ ] **Step 5: 顺带钉住「审计结果不进导出产物」。** 这一期 `src/main/exporters/` 一行不改，所以最省事也最可靠的断言是：**三种格式各导一次**，每一份都 `not.toContain('redactionReport')`、`not.toContain('打掉了什么')`。三种都导而不是只导一种，因为三条渲染路径是各写各的——只验 markdown 的话，将来某一天 json 那条路上多带出一个字段，这条测试是绿的。`renderExport` 是个纯函数（不碰 Electron 的保存对话框），所以这件事在测试里就是一次普通调用。B4 要往导出里加汇总计数时，会先撞到这条测试，那时它该被改成「只允许计数进去」——这正是我们希望那次改动被人看见的方式。
